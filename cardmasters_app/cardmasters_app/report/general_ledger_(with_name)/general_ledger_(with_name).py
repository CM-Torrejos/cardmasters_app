...
def get_gl_entries(filters, accounting_dimensions):
    ...
    for gl_entry in gl_entries:
        if gl_entry.party_type and gl_entry.party:
            ...
            name = party_name_map.get(gl_entry.party_type, {}).get(gl_entry.party)
            gl_entry.party_name = name
            
            # --- MODIFICATION --- 
            # If the party is an Employee, assign the name to our dedicated employee_name field
            if gl_entry.party_type == "Employee":
                gl_entry.employee_name = name
            else:
                gl_entry.employee_name = ""
            # --------------------
...
def get_columns(filters):
    ...
    columns += [
        ...
        {"label": _("Party Code"), "fieldname": "party", "width": 100},
    ]
    
    # --- THIS IS THE NEW DEDICATED EMPLOYEE NAME COLUMN ---
    columns.append(
        {

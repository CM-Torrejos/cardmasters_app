	...
	group_wise_columns = frappe._dict(
		{
			...
			"project": ["project", "cost_accuracy", "base_amount", "buying_amount", "gross_profit", "gross_profit_percent"],
			"cost_center": [
				"cost_center",
				"cost_accuracy",
				"base_amount",
	...
	column_map = frappe._dict(
		{
			...
			"manufacture_entry": { 
				...
				"fieldname": "manufacture_entry",
				"fieldtype": "Link",
				"options": "Stock Entry",
				"width": 140,
			},
			"cost_accuracy": {
				"label": _("Cost Accuracy"),
				"fieldname": "cost_accuracy",
				"fieldtype": "Data",
				"width": 190,
			},
			"warehouse": {
...
class GrossProfitGenerator:
	...
	def get_manufacture_entries(self):
		...
		so_details = list(set([x.so_detail for x in self.si_list if x.get("so_detail")]))
		...
		se_data = frappe.db.sql(
			"""
			SELECT
				wo.sales_order_item AS so_detail,
			...
			WHERE
				wo.sales_order_item IN %(so_details)s
		...
		for d in se_data:
			self.manufacture_entries[d.so_detail] = d.stock_entry

		for row in self.si_list:
			if row.get("so_detail"):
				row.manufacture_entry = self.manufacture_entries.get(row.so_detail)
			else:
				row.manufacture_entry = None
	...
	def process(self):
		...
		for row in reversed(self.si_list):
			...
			# ==========================================
			# FIX: Safe String Fallback for Item Group
			# ==========================================
			item_group = str(row.get("item_group") or "").strip().upper()

			if row.indent == 0.0 and grouped_by_invoice:
				# Keep invoice header rows clean
				row.cost_accuracy = None
			elif item_group in ["RETAIL", "RAW MATERIALS"]:
				row.cost_accuracy = "System Generated (Purchased)"
			elif row.manufacture_entry:
				row.cost_accuracy = "Costed"
			else:
				row.cost_accuracy = "System Generated (Estimated)"
	...
	def update_return_invoices(self, row, sales_invoice_item):
		...
		for returned_item_row in returned_item_rows:
			# Returned item quantities are stateful so an oversized return can spill into
			# the next eligible source row without being applied twice.
			...
			if self.filters.get("group_by") == "Payment Term":
				...
				for i, row in enumerate(self.grouped[key]):
					if i == 0:
						...
						if new_row.get("cost_accuracy") != row.get("cost_accuracy"):
							new_row.cost_accuracy = "Mixed"
	...
	def load_invoice_items(self):
		...
		if self.filters.include_returned_invoices:
			invoice_conditions = " and (is_return = 0 or (is_return=1 and return_against is null))"
		else:
			invoice_conditions = " and is_return = 0"
		...
		self.si_list = frappe.db.sql(
			invoice_query.format(
				conditions=conditions + invoice_conditions,
		...
		vouchers_to_ignore = tuple(row.parent for row in self.si_list)
		return_conditions = conditions + " and is_return = 1 and return_against is not null"
		if vouchers_to_ignore:
			self.filters.vouchers_to_ignore = vouchers_to_ignore
			return_conditions += " and return_against not in %(vouchers_to_ignore)s"

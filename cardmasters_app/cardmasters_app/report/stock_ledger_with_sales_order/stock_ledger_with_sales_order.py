...
def execute(filters=None):
    ...
    for sle in sl_entries:
        ...
        # --- CUSTOM MODIFICATION START: Extract SO directly from standard SLE batch convention ---
        if filters.get("segregate_serial_batch_bundle") and sle.get("batch_no"):
            # Splitting '{so_name} - {itemcode}:{itemspecifics}' to isolate '{so_name}'
            sle["sales_order_reference"] = sle.batch_no.split(" - ")[0]
...
def get_segregated_bundle_entries(sle, bundle_details, batch_balance_dict, filters):
    ...
    for row in bundle_details:
        ...
        # --- CUSTOM MODIFICATION START: Extract SO from bundled entries ---
        if filters.get("segregate_serial_batch_bundle") and row.get("batch_no"):
            new_sle["sales_order_reference"] = row.batch_no.split(" - ")[0]
        # --- CUSTOM MODIFICATION END ---
...
def get_columns(filters):
    ...
    # --- CUSTOM MODIFICATION START: Conditionally Add Sales Order Column ---
    if filters.get("segregate_serial_batch_bundle"):
        # Locate the index of the batch_no column so we can insert Sales Order right after it
        batch_index = next((i for i, col in enumerate(columns) if col.get("fieldname") == "batch_no"), -1)
        
        so_column = {
            "label": _("Sales Order Reference"),
        ...
        if batch_index != -1:
            columns.insert(batch_index + 1, so_column)
        else:
            columns.append(so_column)
    # --- CUSTOM MODIFICATION END ---

...
def run_employee_ar_reclassification(filters=None):
	...
	for index, raw_row in enumerate(report_rows):
		...
		try:
			if v_type == "Journal Entry":
				je_name = _process_journal_entry_reclass(v_no, party, r_account, linked_employee, outstanding)
			...
			reconciliation_queue.add((company, party, r_account))
	...
	for comp, pty, acc in reconciliation_queue:
		_trigger_auto_reconcile(comp, pty, acc)
	
	frappe.enqueue(
		method="cardmasters_app.cardmasters_app.doctype.ar_reclassification_tool.ar_reclassification_tool._drain_reconciliation_queue",
		queue="long",
		timeout=1500
	)
...
def _process_journal_entry_reclass(v_no, party, r_account, linked_employee, outstanding):
	# 1. Fetch exact lines from the original Journal Entry (Ordered by row number)
	original_lines = frappe.get_all(
	...
	remaining_to_move = abs(flt(outstanding))

	# 2. Mirror lines 1-to-1 up to the outstanding amount
	for line in original_lines:
		if remaining_to_move <= 0:
			break

		orig_debit = flt(line.debit_in_account_currency)
		orig_credit = flt(line.credit_in_account_currency)
		
		# The amount on this specific row
		line_amount = orig_debit if orig_debit > 0 else orig_credit
		if line_amount == 0:
			continue

		is_line_debit = orig_debit > 0
		
		# We only move up to the remaining outstanding balance (handles partial payments cleanly)
		amount_to_move = min(line_amount, remaining_to_move)

		# Build Customer Row (Reversing the original to clear it)
		customer_debit = amount_to_move if not is_line_debit else 0
		customer_credit = amount_to_move if is_line_debit else 0
		
		row_customer = _build_je_account_row(r_account, "Customer", party, customer_debit, customer_credit)
		
		# If original was a debit, our new row is a credit. We CAN natively link credits!
		if is_line_debit:
			row_customer.update({
				"reference_type": "Journal Entry",
				"reference_name": v_no,
				"reference_detail_no": line.name
			})

		# Build Employee Row (Copying the original to transfer it)
		employee_debit = amount_to_move if is_line_debit else 0
		employee_credit = amount_to_move if not is_line_debit else 0
		...
		# Deduct this row's amount from the total left to move
		remaining_to_move -= amount_to_move

	if not je.get("accounts"):
		 raise Exception("No valid lines could be extracted for reclassification.")
...
def _trigger_auto_reconcile(company, party, account):
	"""
	Programmatically creates and submits the native 
	'Process Payment Reconciliation' DocType for a specific party.
	"""
	try:
		doc = frappe.get_doc({
		...
		frappe.log_error(title=f"Auto Reconcile Failed for {party}", message=str(e))

def _drain_reconciliation_queue():
	"""
	Loops the native ERPNext batch reconciler until the queue is 100% empty.
	Runs in a background worker to prevent UI freezing.
	"""
	...
	while frappe.db.count("Process Payment Reconciliation", {"status": "Queued"}) > 0:
		try:
			trigger_reconciliation_for_queued_docs()
			frappe.db.commit()

		except Exception as e:
			frappe.log_error(title="Queue Drainer Error", message=str(e))
			break
...
def set_batch_no_for_fg_on_manufacture_entry(doc, method):
	...
	sales_order_item_ref = wo.sales_order_item
	
	item_specifics_ref = None
	item_code_ref = None

	if sales_order_item_ref:
		# Pass a list of fields and use as_dict=True to get a dictionary back
		...
		# Safely assign the values if data was found
		if so_item_data:
			# --- START VALIDATION CHECK ---
			mismatches = []
			
			# Note: Work Order standard field for the item is 'production_item'
			if so_item_data.get("item_code") != wo.production_item:
				mismatches.append(
					f"<b>Item Code:</b> SO Item ({so_item_data.get('item_code')}) vs Work Order ({wo.production_item})"
				)
				
			if so_item_data.get("custom_item_specifics") != wo.custom_item_specifics:
				mismatches.append(
					f"<b>Item Specifics:</b> SO Item ({so_item_data.get('custom_item_specifics')}) vs Work Order ({wo.custom_item_specifics})"
				)
				
			if so_item_data.get("custom_particulars") != wo.custom_particulars:
				mismatches.append(
					f"<b>Particulars:</b> SO Item ({so_item_data.get('custom_particulars')}) vs Work Order ({wo.custom_particulars})"
				)
				
			if mismatches:
				error_msg = _("Cannot submit Stock Entry. The following fields do not match between the Sales Order Item and Work Order.<br><br>Please contact the sales order and work order owners to reconcile the discrepancy.<br><br>{0}").format("<br>".join(mismatches))
				frappe.throw(error_msg, title=_("Data Mismatch"))
			# --- END VALIDATION CHECK ---

			item_specifics_ref = so_item_data.get("custom_item_specifics")
			item_code_ref = so_item_data.get("item_code")

frappe.ui.form.on('Petty Cash Count', {
	onload: function(frm) {
    	...
    	if (frm.doc.__islocal && !frm.doc.petty_cash_count.length) {
        	const default_items = ['1000','500','200','100','50', '20', '10', '5', '1', '0.25', '0.05', '0.01'];
       	 
        	default_items.forEach(denom => {
            	const row = frm.add_child('petty_cash_count');
            	row.denomination = denom;
        	});
       	 
        	frm.refresh_field('petty_cash_count');
    	}
	},
	...
	total_petty_cash_count: function(frm) {
    	let total = 0;
    	frm.doc.petty_cash_count.forEach(function(row) {
